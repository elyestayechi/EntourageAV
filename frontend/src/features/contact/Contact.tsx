...
export function Contact() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (formElements) {
        gsap.fromTo(formElements, { opacity: 0, y: 50 }, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn('API submit failed, using localStorage fallback', err);
  ...
  return (
    <section
      ...
      style={{
        clipPath: `polygon(0 1.8rem, calc(50% - 30%) 1.8rem, calc(50% - 25%) 1.2rem, calc(50% + 25%) 1.2rem, calc(50% + 30%) 1.8rem, 100% 1.8rem, 100% 100%, 0 100%)`,
        willChange: 'clip-path',
      }}
    >
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 120% 100% at 50% 0%, rgba(255,255,255,0.12) 0%, transparent 60%)', mixBlendMode: 'soft-light' }} />
      <FilmGrainTexture />
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(180deg, rgba(255,252,248,0.08) 0%, transparent 50%)', mixBlendMode: 'overlay' }} />
      ...
      <div className="container mx-auto px-8 sm:px-4 relative z-10 max-w-[1400px]">
        <div className="mb-12 sm:mb-16 text-center">
          ...
          <h2 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-white leading-[0.9] mb-2">RÉALISONS VOTRE</h2>
        ...
        <div className="max-w-3xl mx-auto">
          <div ref={formRef}>
            ...
            <div data-form-element className="flex flex-wrap justify-center gap-8 mb-12">
              <div className="flex items-center gap-3 text-white/70"><Mail className="w-5 h-5" /><span>contact@entourageav.com</span></div>
              <div className="flex items-center gap-3 text-white/70"><Phone className="w-5 h-5" /><span>+33 1 23 45 67 89</span></div>
              <div className="flex items-center gap-3 text-white/70"><MapPin className="w-5 h-5" /><span>Paris, France</span></div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div data-form-element>
                <textarea
                  name="message" placeholder="Décrivez votre projet..."
              ...
              <div className="flex justify-center">
                <button
                  data-form-element type="submit" disabled={submitting}
                  className="btn-chamfered group relative flex items-center gap-3 px-10 py-5 text-white transition-all duration-500 hover:scale-105 overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              {submitError && (
                <div className="p-4 rounded-2xl text-center text-red-400" style={{ background: 'rgba(255,80,80,0.1)', border: '1px solid rgba(255,80,80,0.2)' }}>
                  {submitError}
                </div>
              )}
