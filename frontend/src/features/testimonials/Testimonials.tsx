    ...
    >
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 relative z-10 max-w-[1400px]">
        <div className="flex flex-col items-center text-center mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight mb-3 md:mb-4" style={{ color: '#2A2522' }}>
            Ce Que Disent Nos Clients
          </h2>
          <div
            className="inline-flex px-6 py-3 max-w-2xl"
            style={{ background: 'rgba(255,255,255,0.3)', backdropFilter: 'blur(40px) saturate(180%)', WebkitBackdropFilter: 'blur(40px) saturate(180%)', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', border: '1px solid rgba(255,255,255,0.25)', clipPath: 'polygon(8px 0, calc(100% - 8px) 0, 100% 8px, 100% calc(100% - 8px), calc(100% - 8px) 100%, 8px 100%, 0 calc(100% - 8px), 0 8px)' }}
          >
        ...
        <div ref={cardsContainerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10 mt-16" style={{ perspective: '2000px' }}>
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div
                ...
                style={{ background: '#FFFFFF', boxShadow: '0 8px 32px rgba(42,42,42,0.15), 0 4px 16px rgba(42,42,42,0.1)', border: '1px solid rgba(90,90,90,0.1)' }}
              >
                <div className="mb-6">
                  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-10 h-10" style={{ fill: '#2A2A2A', filter: 'drop-shadow(0 2px 10px rgba(42,42,42,0.4))' }}>
                ...
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`w-5 h-5 ${i < testimonial.rating ? 'fill-current' : ''}`} style={{ color: i < testimonial.rating ? '#2A2A2A' : '#e5e7eb', filter: i < testimonial.rating ? 'drop-shadow(0 0 8px rgba(42,42,42,0.3))' : 'none' }} />
                  ))}
                </div>

                <div className="mb-8 flex-1">
                  <p className="leading-relaxed italic text-base lg:text-lg" style={{ color: '#5A5A5A', textShadow: '0 1px 10px rgba(42,42,42,0.2)' }}>
                    "{testimonial.text}"
                  </p>
                </div>

                <div>
                  <div className="w-12 h-px mb-4" style={{ background: 'linear-gradient(90deg, #2A2A2A 0%, transparent 100%)' }} />
                  <div className="font-bold text-lg mb-1" style={{ color: '#2A2522' }}>{testimonial.name}</div>
                  <div className="text-sm mb-2" style={{ color: '#5A5A5A' }}>{testimonial.location}</div>
                  {testimonial.project && (
                    <div className="text-xs uppercase tracking-wider" style={{ color: '#2A2A2A' }}>{testimonial.project}</div>
                  )}
                </div>
              </div>
            </div>
